...
public class XVisitorOutlinePage extends DslOutlinePage {
	...
	private List<DslFilterAction> buildActions(DslFilterActionGroup group) {
		XVisitorImages provider = XVisitorUI.getDefault().getImageProvider();
		...
		ImageDescriptor ruleDesc = provider.DESC_OBJ_PARSER_FILTER;
		...
		ImageDescriptor tokenDesc = provider.DESC_OBJ_LEXER_FILTER;
		...
		ImageDescriptor optDesc = provider.DESC_OBJ_OPTIONS_FILTER;
		...
		ImageDescriptor atDesc = provider.DESC_OBJ_ACTIONS_FILTER;
